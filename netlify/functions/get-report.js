...
exports.handler = async (event) => {
  ...
  const { POSTGREST_URL } = process.env;
  if (!POSTGREST_URL) return json(500, { error: 'Not configured.' });

  try {
    const r = await fetch(
      `${POSTGREST_URL}/report_cards?select=full_name,primary_job,secondary_job,composite_grade,composite_score,first_read,report&token=eq.${encodeURIComponent(token)}&limit=1`
    );
